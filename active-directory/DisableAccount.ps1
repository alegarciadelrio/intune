...
$email = Read-Host 'Enter the email (For example james.guo@company.com)'
...
Invoke-Command SERVER-AZUREAD-W19V -Credential BWSRVR.CORP\Administrator { Start-ADSyncSyncCycle -PolicyType Delta }
...
Get-ADUser "z_archive.$name"| Move-ADObject -TargetPath 'OU=Archived Mailboxes,OU=Users,OU=SITE-MARRICKVILLE,DC=BWSRVR,DC=CORP'
